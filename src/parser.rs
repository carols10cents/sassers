        ...
        while let Some(Ok(op_or_token)) = self.tokenizer.next() {
            match op_or_token {
                OperatorOrToken::Token(TokenOffset {
                    token: Token::String(ref string_val),
                    offset: off,
                    ...
                    if let Err(e) = Parser::expect_operator(Operator::Colon, self.tokenizer.next())
                    {
                        return Some(Err(e));
                    };
                    ...
                    return Some(Ok(Root::Variable(SassVariable {
                        name: variable_name,
                ...
                OperatorOrToken::Token(
                    ...
                    if ambiguous_holding_pen.is_empty() {
                        return Some(Ok(Root::Comment(SassComment { content: t })));
                    }
                }
                _ => match ambiguous_holding_pen.pop() {
                    Some(held) => {
                        ambiguous_holding_pen.push(held.combine(&op_or_token));
                    }
                    None => {
...
impl<'a> Parser<'a> {
    ...
    pub fn expect_operator(
        expected: Operator,
        actual: Option<Result<OperatorOrToken>>,
    ) -> Result<OperatorOrToken> {
        match actual {
            None => Err(SassError {
                offset: 0,
                kind: ErrorKind::UnexpectedEof,
                message: format!("Expected to see `{}`, instead reached EOF.", expected,),
            }),
            Some(res) => match res {
                Err(e) => Err(e),
    ...
    pub fn parse_body(&mut self) -> Result<Vec<Node>> {
        ...
        while let Some(Ok(op_or_token)) = self.tokenizer.next() {
            match op_or_token {
                ...
                OperatorOrToken::Operator(OperatorOffset {
                    ...
                    let value = Expression::parse(&mut self.tokenizer)?;
                ...
                OperatorOrToken::Token(
                    ...
                    if rule_stack.is_empty() && ambiguous_holding_pen.is_empty() {
                        body.push(Node::Comment(SassComment { content: content }));
                    } else if !rule_stack.is_empty() {
                        // TODO: mut ref to last?
                        let mut rule = rule_stack.pop().unwrap();
                        rule.children
                            .push(Node::Comment(SassComment { content: content }));
                        rule_stack.push(rule);
                    }
                }
                _ => match ambiguous_holding_pen.pop() {
        ...
        Err(SassError {
            offset: 0,
            kind: ErrorKind::UnexpectedEof,
            message: String::from("Expected to see rule body ending in `}`, instead reached EOF."),
        })
    }
}
...
mod tests {
    ...
    fn it_returns_nested_rules() {
        ...
        assert_eq!(
            ...
            Some(Ok(Root::Rule(SassRule {
                ...
                children: vec![Node::Rule(SassRule {
                    selectors: vec![
                        TokenOffset {
    ...
    fn it_returns_variable_setting_statements() {
        ...
        assert_eq!(
            parser.next(),
            Some(Ok(Root::Variable(SassVariable {
                name: TokenOffset {
                    token: Token::String("$color".into()),
                    offset: Some(0)
                },
    ...
    fn it_returns_a_comment() {
        ...
        assert_eq!(
            parser.next(),
            Some(Ok(Root::Comment(SassComment {
                content: TokenOffset {
                    token: Token::Comment("/* hi */".into()),
                    offset: Some(0)
                },
            })))
        );
