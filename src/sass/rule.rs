...
impl Streamable for SassRule {
    fn stream(&self, output: &mut Write, style: &SassOutputStyle) -> Result<()> {
        self.recursive_stream(output, style, "", "")?;
        Ok(write!(output, "{}", style.rule_separator())?)
    }
}
...
impl SassRule {
    ...
    pub fn recursive_stream(
        ...
        if has_properties {
            ...
            first_prop.stream(output, style)?;
            ...
            for prop in properties {
                ...
                prop.stream(output, style)?;
            }
            write!(output, "{}}}", style.property_brace_separator())?;
        }
        ...
        if let Some(cr) = child_rules.next() {
            ...
            if has_properties {
                ...
                write!(
                    output,
                    "{}",
                    style.rule_and_child_rules_separator(&recursive_nesting)
                )?;
            }
            cr.recursive_stream(output, style, &selector_string, &recursive_nesting)?;
            for cr in child_rules {
                write!(
                    output,
                    "{}",
                    style.child_rule_separator(has_properties)
                )?;
                cr.recursive_stream(output, style, &selector_string, &recursive_nesting)?;
            }
        }
    ...
    pub fn optimize(self) -> Vec<SassRule> {
        ...
        if self.has_properties() {
            ...
            self.child_rules()
                .into_iter()
                .flat_map(|cr| cr.collapse_with_parent_selectors(&self.selectors))
                .collect::<Vec<_>>()
        }
    }

    pub fn collapse_with_parent_selectors(self, parents: &Vec<TokenOffset>) -> Vec<SassRule> {
        let new_selectors = parents
            .iter()
            .flat_map(|p| {
                self.selectors
                    .iter()
        ...
        SassRule {
            ...
            children: self
                ...
                .filter_map(|c| match c {
                    ...
                    Node::Property(lex, ex) => Some(Node::Property(
                        lex,
                        ExpressionEvaluator::evaluate(ex, &local_context),
                    )),
                    Node::Comment(sc) => Some(Node::Comment(sc)),
                    Node::Variable(sv) => {
                        let evaluated_var = ExpressionEvaluator::evaluate(sv.value, &local_context);
    ...
    fn it_collapses_subrules_without_properties() {
        let innermost_rule = SassRule {
            ...
            children: vec![Node::Property(
                TokenOffset {
                    token: Token::String("font-weight".into()),
                    offset: Some(12),
                },
        ...
        assert_eq!(
            ...
            vec![SassRule {
                selectors: vec![TokenOffset {
                    token: Token::String("div span img".into()),
                    offset: Some(0)
                },],
                children: vec![
                    Node::Property(
